...
def predict_batch(model, inputs):
    ...
    # 前向传播
    with torch.no_grad():
        outputs = model(**inputs)   # outputs.logits 形状（batch_size, 2）
    # 转换为预测概率
    batch_results = torch.softmax(outputs.logits, dim=-1)   # 形状（batch_size, 2）
    return batch_results[:, 1].tolist() # 转换成列表返回，形状（batch_size, ）
